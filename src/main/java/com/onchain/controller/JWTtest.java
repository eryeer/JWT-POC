...
public class JWTtest {
    ...
    private static final long ADVANCE_EXPIRE_TIME = 5 * 60 * 1000;
    private static final long OLD_TOKEN_EXPIRE_TIME = 5 * 60 * 1000;
    private static final long JWT_EXPIRE_TIME_LONG = 7 * 60 * 1000;
    @Autowired(required = false)
    private RedisTemplate<String, String> redisTemplate;
    private String jwtVersion = "0.1";
...
//                .setClaims(claims)          //如果有私有声明，一定要先设置这个自己创建的私有的声明，这个是给builder的claim赋值，一旦写在标准的声明赋值之后，就是覆盖了那些标准的声明的
    ...
    private SecretKey createSecretKey() {
        byte[] encodedKey = "12345677".getBytes();
